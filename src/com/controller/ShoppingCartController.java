...
public class ShoppingCartController {
	...
	public ModelAndView ordernow(HttpServletRequest req, ModelMap mm, HttpSession session) {
		...
		System.out.println(req.getParameter("qid"));
		Quote p = this.quoteService.getQuote(id);
		//p.setProductQuantity(quantity);
		if (session.getAttribute("cart") == null) {
			List<Items> cart = new ArrayList<Items>();
			cart.add(new Items(p));
		...
		ModelAndView mav = new ModelAndView("redirect:/displayAllQuoteToCustomer.jsp");
		return mav; 
	}
	...
	private int isExisting(int id, HttpSession session) {
		...
		for (int i = 0; i < cart.size(); i++)
			...
			if (cart.get(i).getQuote().getQid() == id)
				return i;
	...
	@SuppressWarnings("unchecked")
	public ModelAndView placeOrder( @PathVariable("totalAmount") int totalAmount,HttpSession session) {
		ModelAndView mav = new ModelAndView();
		List<Items> listOfItem = (List<Items>)session.getAttribute("cart");
		String userEmail = (String)session.getAttribute("user");
		//quoteService.reduceQuantity(listOfItem);
